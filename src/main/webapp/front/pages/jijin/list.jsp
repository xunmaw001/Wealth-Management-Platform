                    ...
                    <form class="layui-form" lay-filter="myForm">
                      ...
                      <div class="layui-inline" style="margin-bottom: 10px;">
                          ...
                          <div class="layui-input-inline">
                              <input type="text" name="jijinName" id="jijinName" placeholder="基金名称" autocomplete="off" class="layui-input">
                          </div>
                      </div>
                      ...
                      <div class="layui-inline" style="margin-bottom: 10px;">
                          ...
                          <div class="layui-input-inline">
                              <select name="jijinTypes" id="jijinTypes" lay-filter="jijinTypes">
                                  <option value="">--请选择--</option>
                                  <option v-for="item in jijinTypesList" :value="item.codeIndex" :key="item.codeIndex">{{ item.indexName }}</option>
                              </select>
                          </div>
                      </div>
     
                      <div class="layui-inline" style="margin-bottom: 10px;">
                          <label class="layui-form-label">基金风险类型</label>
                          <div class="layui-input-inline">
                              <select name="jijinFengxianTypes" id="jijinFengxianTypes" lay-filter="jijinFengxianTypes">
                                  <option value="">--请选择--</option>
                                  <option v-for="item in jijinFengxianTypesList" :value="item.codeIndex" :key="item.codeIndex">{{ item.indexName }}</option>
                              </select>
                          </div>
                      </div>
                        ...
                        <ul>
                            <li v-for="(item,index) in dataList" v-bind:key="index">
                                <div class="ng-prslist-doc">
                                    <div class="ng-prslist-img" @click="jumpCheck('../jijin/detail.jsp?id='+item.id , item.aaaaaaaaaa == null?'':item.aaaaaaaaaa , item.shangxiaTypes == null?'':item.shangxiaTypes)" >
                                        <a href="#" class="ng-prsliik">
                                            <span class="ng-prsliik-ki ng-prsliik-ki_1">
                                                <img class="swiper-item" :src="item.jijinPhoto?item.jijinPhoto.split(',')[0]:''" style="height: 350px;width:100%;"/>
                                            </span>
                                    ...
                                    <div class="ng-prslist-tit" @click="jumpCheck('../jijin/detail.jsp?id='+item.id , item.aaaaaaaaaa == null?'':item.aaaaaaaaaa , item.shangxiaTypes == null?'':item.shangxiaTypes)" ><span>{{item.jijinName}}</span></div>
                                    <div class="ng-prslist-tit" v-if="item.jijinNewMoney"><span style="color: red;">{{item.jijinNewMoney}} RMB</span></div>
                                </div>
                            </li>
                        </ul>
                    </div>
...
<script>
    var vue = new Vue({
        ...
        // 当前表字典表数据
            jijinTypesList : [],
            jijinFengxianTypesList : [],
            jijinTuijianTypesList : [],
            shangxiaTypesList : [],
            jijinTypes: 0,//类型
            limit: 8,
    ...
    // 当前表的字典表
        ...
        function jijinTypesSelect() {
            http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=jijin_types", 'get', {}, function (res) {
                if(res.code == 0){
                    vue.jijinTypesList = res.data.list;
                }
            });
        }
        // 基金风险类型的查询
        function jijinFengxianTypesSelect() {
            http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=jijin_fengxian_types", 'get', {}, function (res) {
                if(res.code == 0){
                    vue.jijinFengxianTypesList = res.data.list;
                }
            });
        }
        // 首页推荐的查询
        function jijinTuijianTypesSelect() {
            http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=jijin_tuijian_types", 'get', {}, function (res) {
                if(res.code == 0){
                    vue.jijinTuijianTypesList = res.data.list;
                }
            });
        }
        // 是否上架的查询
        ...
        function pageList() {
            ...
            if (jquery('#jijinName').val() != null && jquery('#jijinName').val() != "" && jquery('#jijinName').val() != "null" ) {
                param['jijinName'] = jquery('#jijinName').val();
            }
                 
            var jijinTypes = $("#jijinTypes").find("option:selected").val();
            if (jijinTypes != null && jijinTypes != "" && jijinTypes != "null") {
                param['jijinTypes'] = jijinTypes;
            }
         
            var jijinFengxianTypes = $("#jijinFengxianTypes").find("option:selected").val();
            if (jijinFengxianTypes != null && jijinFengxianTypes != "" && jijinFengxianTypes != "null") {
                param['jijinFengxianTypes'] = jijinFengxianTypes;
            }
                                                        

            // 获取列表数据
            http.request('jijin/list', 'get', param, function(res) {
