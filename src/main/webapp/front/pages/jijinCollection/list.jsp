            ...
            <div class="ng-inmod-ini">
                ...
                <div class="ng-inside-box ng-fault ng-container" aos="fade-up">
                    <div class="team agileits" id="team">
                        <div class="team-info">
                            <div class="team-row">
                                <div v-for="(item,index) in dataList" v-bind:key="index" class="col-md-3 col-sm-6 col-xs-6 team-grids">
                                    <i @click="deleteJijinCollection(item.id)" class="layui-icon" style="float:right;color:red;margin-top: 0px;margin-right: 3px;font-size:20px">&#xe640;</i>
                                    <div class="team-agile-img" @click="jumpCheck('../jijin/detail.jsp?id='+item.jijinId , item.jijinDelete == null?'':item.jijinDelete , item.shangxiaTypes == null?'':item.shangxiaTypes)">
                                        <img :src="item.jijinPhoto?item.jijinPhoto.split(',')[0]:''" style="height: 350px;width:100%;" />
                                        <div class="view-caption">
                                            <div class="w3ls-info">
                                                <h4>{{item.jijinName}}</h4>
                                            </div>
                                            <b v-if="item.jijinNewMoney" style="float: right;display: inline-block;color: #FFFFFF;font-size: 17px;">
                                                {{item.jijinNewMoney}} RMB
                                            </b>
...
<script>
    var vue = new Vue({
        ...
        methods: {
            ...
            ,deleteJijinCollection(id){
                layui.layer.confirm('确认要删除这个自选吗？', {
                    btn : [ '确定', '取消' ]//按钮
                }, function(index) {
                    var paramArray = [];
                    paramArray.push(id);
                    layui.http.requestJson('jijinCollection/delete', 'post',paramArray, function (res) {
                        if (res.code == 0) {
                            layer.msg('删除成功', {
                                time: 2000,
                                icon: 6
    ...
    // 级联表的字段数据
        ...
        function pageList() {
            ...
            // 获取列表数据
            http.request('jijinCollection/page', 'get', param, function(res) {
